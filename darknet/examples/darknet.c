...
//char** split(char params, char delimiter)
char** split(char params[100], char delimiter[2])
{

	int i = 0;
	int j = 0;
	char *token = strtok(params, delimiter);
	char darknet_params[4][100];
	while (token != NULL)
	{

		strcpy(darknet_params[i], token);		
        	token = strtok(NULL, delimiter);
		i++;
    	}
	
	return darknet_params;
}
...
printf("\nargv 5 = %s\n", argv[5]);
	...
	int j = 0;
	int i = 0;
	char darknet_params[4][400];
	int num_objects;
	char delimiter[2];
	strcpy(delimiter, " ");
	char options[400];
	strcpy(options, argv[2]);
	char* objectArray[10][80];
	char* tagNameArray[10];
	char* markerNameArray[10];

	char *token = strtok(options,delimiter);
	...
	json_object* jobj_canvas_params = json_object_object_get(jobj, "frameWidth");
	json_object* jobj_canvas_width = json_object_object_get(jobj_canvas_params, "width");
	...
	char* feature = json_object_get_string(jobj_feature);
	int image_height = json_object_get_int(jobj_image_height);
	int image_width = json_object_get_int(jobj_image_width);

	json_object* jobj_camID = json_object_object_get(jobj, "camId");
	char* camID = json_object_get_string(jobj_camID);

	json_object* streamingUrl_obj = json_object_object_get(jobj, "streamingUrl");	
	char* streamingUrl = json_object_get_string(streamingUrl_obj);

	json_object* jobj_deviceName = json_object_object_get(jobj, "deviceName");
	char* deviceName = json_object_get_string(jobj_deviceName);

	json_object* jobj_userId = json_object_object_get(jobj, "userId");
	char* userId = json_object_get_string(jobj_userId);

	json_object* jobj_bounding_boxes_array = json_object_object_get(jobj, "boundingBox");
	char* bounding_boxes = json_object_get_string(jobj_bounding_boxes_array);

	int arraylen = json_object_array_length(jobj_bounding_boxes_array);
	...
	for (int i = 0; i < arraylen; i++)
	{
		...
		char* bbox_shape = json_object_get_string(json_object_object_get(bbox_object, "shape"));
		if (strcmp(bbox_shape, "Rectangle") == 0)
		{
			bounding_box_params_array[i][0] = json_object_get_string(json_object_object_get(bbox_object, "x"));

			bounding_box_params_array[i][1] = json_object_get_string(json_object_object_get(bbox_object, "y"));

			bounding_box_params_array[i][2] = json_object_get_string(json_object_object_get(bbox_object, "x2"));
		...
		else if(strcmp(bbox_shape, "Line") == 0)
		{
			...
			//bounding_box_params_array[i][4] = json_object_get_string(json_object_object_get(bbox_object, "_id"));
			bounding_box_params_array[i][5] = bbox_shape;

			bounding_box_params_array[i][6] = json_object_get_string(json_object_object_get(bbox_object, "direction"));
		}

		if (strcmp(bbox_shape, "Triangle") == 0)
		{
			bounding_box_params_array[i][0] = json_object_get_string(json_object_object_get(bbox_object, "x"));
			...
			bounding_box_params_array[i][5] = bbox_shape;
			bounding_box_params_array[i][7] = json_object_get_string(json_object_object_get(bbox_object, "x3"));
			bounding_box_params_array[i][8] = json_object_get_string(json_object_object_get(bbox_object, "y3"));

		}
			
		tagNameArray[i] = json_object_get_string(json_object_object_get(bbox_object, "tagName"));
		markerNameArray[i] = json_object_get_string(json_object_object_get(bbox_object, "markerName"));
	
		json_object* jobj_objects = json_object_object_get(bbox_object, "detectionObjects");
		num_objects = json_object_array_length(jobj_objects);
		
		
			for (int a = 0;a < num_objects; a++)
			{
				objectArray[i][a] = json_object_get_string(json_object_array_get_idx(jobj_objects, a));
			}
	}

	
//	printf("Canvas width = %d, height = %d/ Image width = %d height = %d/ camId = %s/ devicename = %s / userId = %s", canvas_width, 	canvas_height, image_height, image_width, camID, deviceName, userId);
	char* resultUrl = argv[3];
