...
void* threadFunc(void* final_bounding_boxes_aoiid)
{
		...
		get_region_boxes(l[t], im.w, im.h, net[t]->w, net[t]->h, thresh, probs, boxes, masks, 0, 0, hier_thresh, 1);
		if (nms) do_nms_sort(boxes, probs, l[t].w*l[t].h*l[t].n, l[t].classes, nms);

		draw_detections_heimdall(im, l[t].w*l[t].h*l[t].n, thresh, boxes, probs, masks, names, alphabet, l[t].classes, input, num_bbox, my_data->bounding_boxes, sendResultURL, areathreshold, /*my_data->aoi,*/ device_name, my_data->shape,objectsArray, my_data->num_objects, my_data->direction, t , my_data->tagName, my_data->markerName);
...
void test_detector_heimdall(char *_datacfg, char *cfgfile, char *weightfile, float _thresh, float _hier_thresh, char *_sendResultURL, char folderPath[1000], int _num_bbox, char* bounding_box_params_array[10][2000], int canvas_width, int canvas_height, char *_userId, int img_width, int img_height, char* _deviceName, char* camID, char* objectArray[10][80], int num_objects, char* feature, char* tagNameArray[10], char*  markerNameArray[10])
{
	...
	char watchFileName[1000];
	strcpy(watchFileName , folderPath);	
	strcat(watchFileName, camID);
	strcat(watchFileName, "/");
	printf("\nComplete folder path :: %s\n", watchFileName);
	...
	//Inotify target set:: to the folder of Camera
	  ...
	  wd = inotify_add_watch( fd, watchFileName, IN_CREATE);

	//read to determine the event change happens on “/watchFileName” directory. Actually this read blocks until the change event occurs
	length = read( fd, buffer, buf_len ); 
		...
		if ( event->len ) 
		{
			...
			if ( event->mask & IN_CREATE) 
			{
				...
				else 
				{
					...
					//Got the changed image
					dummy_filename = event->name;
					
					//got_image_time = current_timestamp() 
...
//_____________________________________________________________________________________________________________________________________________
	...
	int c = 0;
	while (c < (strlen(dummy_filename)) - 8) {
		ch_filename[c] = dummy_filename[1+c];
      		c++;
        }
   	ch_filename[c] = '\0';
	...
	printf("\nDetection starting on Image:[%s] ", filename);
	
/*
	strcpy(_ch_filename, dummy_filename); 
 	strcpy(ch_filename, _ch_filename); 
...
/*
	char filename[2000];
	strcpy(filename, watchFileName);
	strcat(filename, dummy_filename);
   //strcat(filename, ch_filename);
*/
	usleep(200000);
...
//threadFunc((void *) &thread_data_array[0]);
